...
def word_freq(data_file_path: Path, column_name: str, stopwords_list: List[str]) -> pd.DataFrame:
    ...
    word_counter_df.to_csv(f"{data_file_path}/word_counter_{column_name}.csv",
        encoding="utf-8-sig",
        index=False,
    )
...
def create_barplot(img_file_path: Path, df: pd.DataFrame) -> None:
    ...
    fig.suptitle("Data taken from subreddits: Ghoststories, Ghosts, Paranormal and ParanormalEncounters")  # or plt.suptitle('Main title')
    ...
    plt.savefig(f"{img_file_path}/{axes.get_ylabel()}.png",
        bbox_inches="tight",
        dpi=150,
    )
    plt.close()
...
def create_plots():
    ...
    df_post_title = word_freq(data_file_path = raw_data_file_path, column_name="post_title", stopwords_list=stopwords_personalized)
    df_post_text = word_freq(data_file_path = raw_data_file_path, column_name="post_text", stopwords_list=stopwords_personalized)
    create_barplot(img_file_path=img_file_path, df=df_post_title)
    create_barplot(img_file_path=img_file_path, df=df_post_text)
    create_wordcloud(data_file_path = raw_data_file_path, img_file_path=img_file_path, column_name="post_title")
    create_wordcloud(data_file_path = raw_data_file_path, img_file_path=img_file_path, column_name="post_text")
